...
export function ERPCRMSpotlight() {
  return (
    <section id="erp" className="relative overflow-hidden bg-background py-24 lg:py-32">
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(78,242,173,0.08)_1px,transparent_1px)] bg-[size:72px_72px] opacity-60" />
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        <div>
          ...
          <Link
            href="#booking"
            className="mt-9 inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-lime px-6 py-3 text-sm font-black uppercase tracking-wide text-slate-950 transition hover:bg-lime/90"
          >
            Build My System
        ...
        >
          <div className="relative overflow-hidden rounded-lg border border-white/10 bg-slate-950 shadow-2xl shadow-[#082d20]/50">
            <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-5 py-4">
              ...
              <span className="rounded bg-lime/10 px-3 py-1 font-mono text-xs uppercase tracking-wider text-lime">
                owned
              </span>
            </div>
            <div className="grid gap-4 p-5 md:grid-cols-[0.8fr_1.2fr]">
              <div className="space-y-3">
                {erpModules.slice(0, 4).map((module) => (
                  ...
                  >
                    <module.icon className="h-4 w-4 text-lime" />
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3">
            {erpModules.map((module, index) => (
              <motion.div
                key={module.label}
                className="flex items-center gap-2 rounded-md border border-lime/20 bg-lime/10 px-3 py-3 text-sm text-lime backdrop-blur"
